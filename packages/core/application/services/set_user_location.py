...
class SetUserLocationService:
    """Updates whichever fields are provided, leaving the rest of the
    owner's existing record untouched - e.g. refreshing `current` from a
    new GPS fix shouldn't erase a previously saved `home`."""
    ...
    def execute(self, data: SetUserLocationInput) -> SetUserLocationOutput:
        ...
        updates = {
            field: getattr(data, field) for field in fields if getattr(data, field) is not None
        }
        if "current" in updates:
            updates["current_captured_at"] = utc_now()
